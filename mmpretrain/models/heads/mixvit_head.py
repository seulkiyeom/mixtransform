...
class MixViTClsHead(ClsHead):
    def __init__(self, in_channels, num_classes, bias=True, std=0.02, drop=0., deploy=False,
                 init_cfg=dict(type='Normal', layer='Linear', std=0.01),
                 *args,
                 **kwargs):
        super(MixViTClsHead, self).__init__(
    ...
    def fuse_parameters(self):
        for child_name, child in self.named_children():
            if hasattr(child, 'fuse'):
                setattr(self, child_name, child.fuse())
            else:
                self.fuse_parameters(child)
