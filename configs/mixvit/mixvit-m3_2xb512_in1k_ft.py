...
_base_ = [
    ...
    '../_base_/schedules/imagenet_bs1024_adamw_mixtransform_finetune.py',  #'schedules/imagenet_bs256.py',
...
model = dict(
    backbone=dict(arch='m3'), #frozen_stages=-1), #, deploy=True), #deploy할때 사용할 것
    head=dict(
        in_channels=320, # deploy=True #deploy할때 사용할 것
    ),
