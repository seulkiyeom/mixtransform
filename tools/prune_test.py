...
def parse_args():
    ...
    parser.add_argument(
        '--pruned',
        action='store_true',
        help='execute if this is pruned model')
    parser.add_argument(
        '--cfg-options',
        nargs='+',
        action=DictAction,
...
def main():
    ...
    if args.pruned: #when testing with pruned_model. It is necessary to test with pruned architecture (not original architecture)
        # runner.load_pruned_model = load_pruned_model.__get__(runner, Runner)
        runner.load_pruned_model()
